...
// ===========================================================================
// Allergy benefit-exposure route  (POST /api/allergy)  - SELF-CONTAINED, Claude.
// Powers demo 3 (CPT 95165 Immunotherapy Benefit Agent): takes a short practice
// profile and maps where the manual benefit-verification work actually goes, in
// hours and dollars a month. Calls the Anthropic Messages API with fetch (no
// SDK) off the same ANTHROPIC_API_KEY secret, so this block can be pasted into
// any Worker. Kept in sync with backend/allergy-benefits-route.snippet.js.
// ===========================================================================
...
export default {
  async fetch(request, env) {
    ...
    // Route: /api/allergy maps an allergy practice 95165 benefit workload.
    if (new URL(request.url).pathname.endsWith("/allergy")) {
      return handleAllergy(body, env, headers, request);
    }
